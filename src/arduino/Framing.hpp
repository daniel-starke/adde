...
class Framing {
public:
	typedef bool (* WriteCallback)(const uint8_t /* val */, const bool /* eof */);
	enum {
...
public:
	 ...
	 */
	explicit Framing(WriteCallback w):
		writer(w),
		size(0),
		state(ST_START),
		firstOut(true)
		...
		case ST_SEP:
			...
			case SEP:
				if (this->size == 0) {
					...
					const uint16_t containedCrc = static_cast<uint16_t>((uint16_t(ptr[0]) << 8) | uint16_t(ptr[1]));
					const uint16_t calculatedCrc = Crc16(this->buffer, ptr);
					fn(this->buffer[0], this->buffer + 1, static_cast<size_t>(this->size - 3), containedCrc != calculatedCrc);
					this->size = 0;
				}
				break;
			default:
	...
	bool beginTransmission(const uint8_t seq = 0) {
		...
		this->crc = Crc16();
		if (this->firstOut || (now - this->lastOut) > 1000) {
			if ( ! this->writer(uint8_t(SEP), false) ) return false;
		}
		this->firstOut = false;
		this->lastOut = now;
	...
	bool endTransmission() {
		...
		if ( ! this->writer(uint8_t(SEP), true) ) return false;
		return true;
	}
	...
	bool write(const uint8_t val) {
		...
		case SEP:
		case ESC:
			if ( ! this->writer(uint8_t(ESC), false) ) return false;
			if ( ! this->writer(uint8_t(val ^ FLIP), false) ) return false;
			break;
		default:
			if ( ! this->writer(val, false) ) return false;
			break;
		}
		return true;
	}
