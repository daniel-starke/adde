...
public:
	...
	Crc16 & operator() (const uint8_t value) {
		this->val = static_cast<uint16_t>(__CRC16_HPP__ROM_READ_U16(crc16Table, (this->val ^  value      ) & 0x0F) ^ (this->val >> 4));
		this->val = static_cast<uint16_t>(__CRC16_HPP__ROM_READ_U16(crc16Table, (this->val ^ (value >> 4)) & 0x0F) ^ (this->val >> 4));
		return *this;
	}
	...
	Crc16 & operator() (It first, const It last) {
		while (first != last) {
			this->operator() (static_cast<uint8_t>(*first));
			++first;
		}
		return *this;
	}
