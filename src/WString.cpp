...
static const char * _charMap = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";


char * itoa_(int val, char * str, int base) {
	...
	int signedVal = (val < 0) ? val : -val;

	*ptr = 0;
	do {
		*--ptr = _charMap[-(signedVal % base)];
		signedVal /= base;
	} while (signedVal);
...
char * ltoa_(long val, char * str, int base) {
	if (str == NULL || base < 2 || base > 36) {
		return NULL;
	}
...
char * utoa_(unsigned int val, char * str, int base) {
	if (str == NULL || base < 2 || base > 36) {
		return NULL;
	}

	char buf[(sizeof(val) * 8) + 1];
	char * ptr = buf + sizeof(buf) - 1;
...
char * ultoa_(unsigned long val, char * str, int base) {
	if (str == NULL || base < 2 || base > 36) {
		return NULL;
	}
